...
// CountingHandler only records how many times it fired, used to prove
// concurrent register() and dispatch never corrupts the handler list.
...
fn (mut h CountingHandler) on_player_join(mut ctx Context[JoinData]) {
	h.hits++
}

fn register_worker(mut bus Bus, times int) {
	for i in 0 .. times {
		bus.register(&CountingHandler{}, .normal)
	}
}

fn dispatch_worker(mut bus Bus, times int) {
	for _ in 0 .. times {
		mut ctx := new_context(JoinData{ message: '' })
		bus.player_join(mut ctx)
	}
...
fn test_concurrent_register_and_dis_not_racing() {
	mut bus := new_bus()
	...
	for _ in 0 .. register_threads {
		threads << spawn register_worker(mut bus, registers_per_thread)
	}
	for _ in 0 .. dispatch_threads {
		threads << spawn dispatch_worker(mut bus, dispatches_per_thread)
	}
	threads.wait()

	assert bus.len() == register_threads * registers_per_thread
}
