...
// due_scheduled_entries removes and returns up to max entries due at or before
// current. Additional due entries remain queued, bounding catchup work after
// stalls and preventing one tick from processing an unbounded backlog.
pub fn (mut w World) due_scheduled_entries(current i64, max int) []ScheduledEntry {
	w.mutex.lock()
	defer {
		w.mutex.unlock()
	}
	mut due := []ScheduledEntry{}
	mut remaining := []ScheduledEntry{}
	for entry in w.scheduled {
		if entry.due <= current && due.len < max {
			due << entry
		} else {
			remaining << entry
		}
