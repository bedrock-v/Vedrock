...
fn (s &NetworkSession) uuid() types.UUID {
	return types.uuid_from_bytes(parse_uuid(s.player.identity.uuid, s.runtime_id))
}
	...
	return &protocol.PlayerListPacket{
		...
		entries: [
			protocol.PlayerListEntry{
				...
				username:         s.player.identity.display_name
				xbox_user_id:     s.player.identity.xuid
				platform_chat_id: ''
				build_platform:   -1
				skin:             default_skin(s.player.identity.display_name)
				is_teacher:       false
				is_host:          false
				is_sub_client:    false
				color:            0xffffffff
				verified:         s.player.identity.xbox_authenticated
			},
		]
	}
}
...
fn (s &NetworkSession) add_player_packet() &protocol.AddPlayerPacket {
	...
	return &protocol.AddPlayerPacket{
		...
		item:              s.player.held_item()
		game_mode:         s.player.game_mode()
