...
pub fn new_world_registry() &WorldRegistry {
	return &WorldRegistry{}
}

// add registers wr under its own world's name.
pub fn (mut r WorldRegistry) add(wr &WorldRuntime) {
	r.mutex.lock()
...
pub fn (mut r WorldRegistry) get(name string) ?&WorldRuntime {
	r.mutex.lock()
	defer {
		r.mutex.unlock()
	}
...
pub fn (mut r WorldRegistry) remove(name string) ?&WorldRuntime {
	r.mutex.lock()
	defer {
		r.mutex.unlock()
	}
...
pub fn (mut r WorldRegistry) names() []string {
	r.mutex.lock()
	defer {
		r.mutex.unlock()
	}
...
pub fn (mut r WorldRegistry) len() int {
	r.mutex.lock()
	defer {
		r.mutex.unlock()
	}
...
pub fn (mut r WorldRegistry) each_runtime() []&WorldRuntime {
	r.mutex.lock()
	defer {
		r.mutex.unlock()
	}
